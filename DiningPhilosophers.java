...
public class DiningPhilosophers extends Application implements PhilosopherEventHandler{
    ...
    private void appendInfoLine(String line) { Platform.runLater(() -> txtInfo.appendText(line + "\n")); }
    ...
    private void initialize()
    {
        ...
        for (int i = 1; i <= count; i++) {
            ...
            Fork rightFork = forks.get(i % count); //mod provides us to make circular algorithm
            Philosopher philosopher;

            //Preventing dead-luck by giving the last philosopher right fork first
            if (i == count)
            {
    ...
    public void forkTaken(Philosopher philosopher, Fork fork, Direction direction) {
        ...
        appendInfoLine(String.format("Philosopher %d has taken the %s fork (%d).",
    ...
    public void forkReleased(Philosopher philosopher, Fork fork, Direction direction) {
        ...
        appendInfoLine(String.format(
                "Philosopher %d has released the %s fork (%d).",
                philosopher.getNumber(),
                direction == Direction.LEFT ? "left" : "right",
                fork.getNumber()));
    ...
    public void eating(Philosopher philosopher) {
        ...
        appendInfoLine(String.format("Philosopher %d eating..", philosopher.getNumber()));
    ...
    public void thinking(Philosopher philosopher) {
        ...
        appendInfoLine(String.format("Philosopher %d thinking..", philosopher.getNumber()));
